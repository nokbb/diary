    ...
    */
    ...
    public function sendEmailResetPassword(SendEmailRequest $request)
    {
        try {
            ...
            Log::error(__METHOD__ . '...ユーザーへのパスワード再設定用メール送信に失敗しました。 request_email = ' . $request->email . ' error_message = ' . $e->getMessage());
            return redirect()->route('form')->with('danger', '処理に失敗しました。時間をおいて再度お試しください。');
        }
    ...
    public function sendComplete()
    {
        ...
        if (session()->pull(self::MAIL_SENDED_SESSION_KEY) !== 'user_reset_password_send_email') {
            return redirect()->route('form')->with('danger', '不正なリクエストです。');
        }

        return view('mails.send-complete');
    }
    ...
    public function edit(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('form')->with('danger', __('パスワードリセットメールに添付されたURLから遷移してください。'));
        }

        return view('login.resetting')->with('userToken', $userToken);
    }
